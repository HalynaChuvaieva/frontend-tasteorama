...
interface ProfileRecipesListProps {
  ...
  fetchMoreFn: (
    page: number
  ) => Promise<{ recipes: Recipe[]; totalPages: number; totalRecipes: number }>;
}
...
export default function ProfileRecipesList({
  ...
  const [totalRecipes, setTotalRecipes] = useState(initialTotalRecipes);
  ...
  return (
    <div className={css.container}>
      ...
      {recipes.length === 0 ? (
        <p className={css.emptyText}>{"You haven't added any recipes yet."}</p>
      ) : (
